        ...
        return (
            <Paper className="w-full rounded-8 border-1">
                ...
                <div className="text-center pt-12 pb-28" style={{overflow: "auto"}}>
                    <Typography
                        className="text-56 leading-none text-purple-dark">{patient ? patient : 0}</Typography>
                    <Typography className="text-16" color="textSecondary"><h1>Patient</h1></Typography>
                </div>
            </Paper>
        );
    }
...
function mapStateToProps({ DashBoardApp, auth }) {
    return {
        patient: DashBoardApp.DashboardReducer.patient,
        user: auth.user
    };
}
